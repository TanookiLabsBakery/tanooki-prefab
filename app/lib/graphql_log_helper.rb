# modified version of this gem https://github.com/hoverinc/graphql-log_helper

class GraphqlLogHelper
  class << self
    ...
    def selection_to_array(selection)
      [
        selection.name,
        if selection.respond_to?(:value) && selection.value.respond_to?(:arguments)
          selection.value.arguments.map { |arg| selection_to_array(arg) }
        elsif selection.respond_to?(:arguments)
          selection.arguments.map { |arg| selection_to_array(arg) }
        elsif selection.respond_to?(:value)
          selection.value
        end
      ]
    end
